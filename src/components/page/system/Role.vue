<template>
    <div class="table">
        ...
        <el-dialog :title="title" :visible.sync="editVisible" width="50%">
         ...
         <!-- 查看修改权限 -->
        <el-dialog :title="title" :visible.sync="PermissionVisible" width="50%">
...
<script>
    export default {
        ...
        methods: {
            ...
            saveEdit() {
                   if(this.isAdd===false){
                     this.$ajax.postu(urlA+'role/addRole',this.form).then((res) => {
                        if (res.msg == "success") {
                    ...
                    this.$ajax.postu(urlA+'role/updateRole',params).then((res) => {
                        if (res.msg == "success") {
            ...
            handleDelete(index, row) {
             ...
             // 确定删除
            deleteRow(){
                 this.$ajax.postu(urlA+'role/deleteRole',{id:this.idx}).then((res) => {
                    if (res.msg == "success") {
            ...
            OpenPermission(index, row){
                ...
                this.$ajax.postu(urlA+'sys/getTreePermission',{role_id:row.id}).then((res) => {
                    if (res.msg == "success") {
                        this.nodes = res.data;
            ...
            savePermission(){
              this.$ajax.postu(urlA+'sys/updatePermission',{role_id:this.perId,json:JSON.stringify(this.newDatas)}).then((res) => {
                    if (res.msg == "success") {
